...
void term_putn_hex_ucase(uint32_t number)
{
  for(int i=0;i<8;++i)
    {
      term_putchar(hex_chars_ucase[((number & 0xF0000000 ) >> 28)]);
      number<<=4;
    }
}
static char hex_chars_lcase[]={'0', '1', '2', '3', '4', '5', '6', '7', '8', '8', 'a', 'b', 'c', 'd', 'e', 'f'};
void term_putn_hex_lcase(uint32_t number)
{
  for(int i=0;i<8;++i)
    {
      term_putchar(hex_chars_lcase[((number & 0xF0000000 ) >> 28)]);
      number<<=4;
    }
}
